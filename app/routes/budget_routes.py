...
from app.models import Budget, Direction, Rubrique, Groupement, ProjetDetail, Consommation
...
def compute_ecart(budget: Budget):
    ...
    total_consomme = sum(
        sum(c.montant or 0 for c in g.consommations)
...
def get_consolidated_report(year):
    sql = text("""
        SELECT 
            ...
            CONCAT(SUM(COALESCE(c.montant, 0)), ' DT') AS consommation,
            b.cloture,
            CASE 
                WHEN b.cloture = 1 THEN CONCAT((g.budget_alloue - COALESCE(SUM(c.montant), 0)), ' DT')
                ELSE '—'
            END AS ecart
        FROM Budgets b
        ...
        LEFT JOIN Consommations c ON c.groupement_id = g.id
        WHERE b.year = :year
        GROUP BY d.name, r.name, g.name, pd.type, pd.montant, b.cloture, g.budget_alloue
